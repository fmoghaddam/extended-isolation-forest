...
private[isolationforest] case class BaggedPoint[Datum](
  ...
  require(
    subsampleWeights.forall(weight => weight >= 0),
    "All values in subsampleWeights Array" +
      " must be >=0"
  )
  ...
  def this(
      ...
      numSubsamples: Int,
      randomState: AbstractIntegerDistribution
  ) = this(
    datum,
    subsampleWeights = {
...
private[isolationforest] case object BaggedPoint {
  ...
  /** Enforces the parameter constraints for BaggedPoint constructors.
    ...
    * @param numSubsamples Number of subsamples of this RDD to take.
    */
  private def enforceRequirements(
      subsamplingRate: Double,
      numSubsamples: Int
  ): Unit = {
    require(
      subsamplingRate > 0 && subsamplingRate <= 1,
      "parameter subsamplingRate must be in" +
        s" the range 0 < subsamplingRate <= 1, but has value of ${subsamplingRate}."
    )
    require(
      numSubsamples > 0,
      s"parameter numSubsamples must be >0, but has value of" +
        s" ${numSubsamples}."
    )
  }
  ...
  def convertToBaggedRDD[Datum](
    ...
    def convertToBaggedRDDHelper[Datum](
      ...
      input.mapPartitionsWithIndex { (partitionIndex, instances) =>
        ...
        instances.map { instance =>
          new BaggedPoint(instance, subsamplingRate, numSubsamples, randomState)
        }
      }
    }
    ...
    if (withReplacement) {
      ...
      if (numSubsamples == 1 && subsamplingRate == 1.0) {
        input.map(datum =>
          BaggedPoint(datum, Array(1))
        ) // Create bagged RDD without sampling
      } else {
  ...
  /** Flattens a baggedRDD to a pair RDD. The key is the subsampleIndex. The instances are
    ...
    */
  def flattenBaggedRDD[Datum](
      baggedRdd: RDD[BaggedPoint[Datum]],
      seed: Long
  ): RDD[(Int, Datum)] = {
    ...
    def roundWeight(subsampleWeight: Double): Int = {
      ...
      if (subsampleWeightDiff == 0)
        ...
        subsampleWeightBase + (if (rnd.nextFloat() < subsampleWeightDiff) 1
                               else 0)
    }
    ...
    baggedRdd.flatMap { baggedPoint =>
      baggedPoint.subsampleWeights.zipWithIndex.flatMap {
        case (subsampleWeight, subsampleId) => {
          Array.fill(roundWeight(subsampleWeight))(
            (subsampleId, baggedPoint.datum)
          )
        }
      }
    }
