...
class ExtendedExtendedIsolationForest(override val uid: String)
  ...
  override def fit(data: Dataset[_]): ExtendedExtendedIsolationForestModel = {
    ...
    // to draw.
    val nSigma = 7.0
    val targetNumSamples =
      numSamples.toDouble + nSigma * math.sqrt(numSamples.toDouble)
    logInfo(
    ...
    val sampleFraction =
      Math.min(targetNumSamples / totalNumSamples.toDouble, 1.0)
    logInfo(
    ...
    val flattenedSampledRdd =
      BaggedPoint.flattenBaggedRDD(sampledRdd, $(randomSeed) + 1)
    ...
    if ($(contamination) > 0.0) {
      ...
      val scores: Dataset[OutlierScore] = isolationForestModel
        .transform(df)
      ...
      val observedContamination = scores
        .map(outlierScore =>
          if (outlierScore.score >= outlierScoreThreshold) 1.0 else 0.0
        )
      ...
      if (
        math.abs(observedContamination - $(contamination)) > verificationError
      ) {
