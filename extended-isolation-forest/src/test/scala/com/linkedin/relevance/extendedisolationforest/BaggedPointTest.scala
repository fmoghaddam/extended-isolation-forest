...
class BaggedPointTest {
  ...
  @Test(
    description = "baggedPointRDDWithSubsamplingWithReplacementFraction1p0Test"
  )
  ...
  @Test(
    description = "baggedPointRDDWithSubsamplingWithReplacementFraction0p5Test"
  )
  ...
  @Test(
    description =
      "baggedPointRDDWithSubsamplingWithoutReplacementFraction1p0Test"
  )
  ...
  @Test(
    description =
      "baggedPointRDDWithSubsamplingWithoutReplacementFraction0p5Test"
  )
  ...
  def flattenBaggedRDDTest(): Unit = {
    ...
    val baggedPointRDD =
      dataPointRDD.map(x => new BaggedPoint(x, subsampleWeights))
    val flattenedBaggedPointRDD =
      BaggedPoint.flattenBaggedRDD(baggedPointRDD, 1L)
    val flattenedBaggedPointArray = flattenedBaggedPointRDD.collect()

    val expectedSumArray =
      expectedResult.map(x => x._1 + x._2.features.sum).sorted
    val actualSumArray =
      flattenedBaggedPointArray.map(x => x._1 + x._2.features.sum).sorted
